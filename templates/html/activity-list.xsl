...
<table title="Aid Transparency - Funding Projects">
	...
	<xsl:for-each select="iati-activity">
		<tr>
		 ...
		 <td><xsl:if test="activity-date[@type='start-actual']"><xsl:value-of select="activity-date[@type='start-actual']/@iso-date"/></xsl:if><xsl:if test="not(activity-date[@type='start-planned'])"><xsl:value-of select="activity-date[@type='start-planned']/@iso-date"/></xsl:if></td>
		 <td><a href="http://tools.aidinfolabs.org/explorer/activity/?activity={iati-identifier}" class="thickbox">View</a></td>
		</tr>
	</xsl:for-each>
</table>
</xsl:template>
